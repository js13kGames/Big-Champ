...
Draw=2;Exit=3;clearColor="#1A1A1AFF";gameLoopFixedTimeStep=1/60;gameLoopFrameTimeAccum=0;previousGameLoopTime=void 0;
...
CreateEnemy=function(){for(var a=0,b=Math.random(),c=0;c<enemyFuncs.length;++c)if(a+=enemyFuncs[c].prob/enemyFuncsTotalProb,b<=a){enemyFuncs[c].func();break}};crowd=[];crowdColors="#331C16 #400101 #2A110A #28251E #152833 #333015 #06280D".split(" ");for(var i=0;300>i;++i)crowd.push({x:Math.random()*gameWidth,y:Math.random()*gameHeight*.5,col:Math.floor(Math.random()*crowdColors.length),off:Math.random(),spd:.003*Math.random()});flashes=[];nextFlashTime=Math.floor(100*Math.random());
...
Player.prototype.Tick=function(){switch(this.state){case PlayerStateIdle:touch.down&&state!=RenderTest&&this.BellyBounce();break;case PlayerStateBelly:this.timer--;0==this.timer&&this.Idle();touch.down&&this.hitConfirm&&this.BellyBounce();break;case PlayerStateHit:this.timer--,0==this.timer&&(0==this.health?(this.bellyOffset.x=10,this.bellyOffset.y=10,nextState=GameOver,this.state=PlayerStateDead):this.Idle())}this.UpdateBellyPhysics()};
...
Player.prototype.OnHit=function(a){0<this.health&&(this.health--,this.timer=PlayerHitTime,this.state=PlayerStateHit,this.bellyOffset.x=5,this.bellyOffset.y=5)};
...
RenderTest=function(a){switch(a){case Enter:player.Reset();CreateEnemy();objs[1].pos.x=600;objs[1].pos.y=300;break;case Tick:player.Tick();touch.down&&(player.state++,player.state>PlayerStateDead&&(player.state=PlayerStateIdle));objs[1].Animate();break;case Draw:DrawBackground(),objs.forEach(function(a){return a.Draw()}),DrawHud()}};nextState=GameState;window.addEventListener("keydown",function(a){"e"==a.key.toLowerCase()&&CreateEnemy()});