...
tkState(Enter)),tkState&&tkState(Tick),touch.down?touch.lastDown=1:++touch.lastDown,touch.up=!1,touch.down=!1;ctx.rect(0,0,gameWidth,gameHeight);ctx.fillStyle=clearColor;ctx.fill();tkState&&tkState(Draw);previousGameLoopTime=a},actualWidth=-1,actualHeight=-1,FitToScreen=function(){var a=canvas.width/canvas.height,c=window.innerWidth,b=window.innerWidth/a;b>window.innerHeight&&(b=window.innerHeight,c=b*a);if(c!==actualWidth||b!==actualHeight)canvas.style.width=c+"px",canvas.style.height=b+"px",actualWidth=
c,actualHeight=b;window.scrollTo(0,0)};window.requestAnimationFrame(GameLoop);var enemySpawnInfo=[];enemySpawnInfo.push({maxPlayerScore:0,possibleEnemyTypes:[0],minSpawnCount:1,maxSpawnCount:1,minSpawnDelay:0,maxSpawnDelay:0,minNextDelay:2,maxNextDelay:2.5});enemySpawnInfo.push({maxPlayerScore:2,possibleEnemyTypes:[0,1,2],minSpawnCount:1,maxSpawnCount:1,minSpawnDelay:0,maxSpawnDelay:0,minNextDelay:1.75,maxNextDelay:1.75});
...
-b,.25*-b,.5*b,d);DrawCircle(.23*b,.25*-b,.5*b,d);DrawRect(.38*-b,.08*-b,b,.5*b,d,45);DrawRect(.38*b,.08*-b,b,.5*b,d,-45);DrawRect(0,.11*b,.75*b,.75*b,d,45);PopMatrix()},DrawHud=function(){for(var a=0;3>a;++a)DrawHeart(50+55*a,40,20,player.health>a?"#F00":"#444");DrawText(player.score.toString(),gameWidth-40,50,40,"#FFF",0,"Arial","Bold","right","center",8);isFrenzy&&DrawText("Wrestler Rush!",.5*gameWidth,.2*gameHeight,36+10*Math.abs(Math.sin(.0075*Date.now())),"#FFD800",-2,"Arial","Bold","center",
"center",10)},particles=[],ParticleTypeHit=0,ParticleTypeDizzy=1,SpawnParticle=function(a,c,b){switch(b){case ParticleTypeHit:b=360*Math.random();particles.push({x:a,y:c,vx:0,vy:0,w:50,h:50,vw:4,vh:4,a:b,va:1,lifetime:10,t:0,c:"#FFF"});particles.push({x:a,y:c,vx:0,vy:0,w:50,h:50,vw:4,vh:4,a:b+45,va:1,lifetime:10,t:0,c:"#FFF"});break;case ParticleTypeDizzy:b=360*Math.random(),particles.push({x:a,y:c,vx:0,vy:0,w:10,h:10,vw:2,vh:2,a:b,va:1,lifetime:10,t:0,c:"#FFF"}),particles.push({x:a,y:c,vx:0,vy:0,
w:10,h:10,vw:2,vh:2,a:b+45,va:1,lifetime:10,t:0,c:"#FFF"})}},DrawParticles=function(){for(var a=0;a<particles.length;++a){var c=particles[a];PushMatrix(c.x,c.y,c.a);var b=1-Math.min(Math.max(c.t/c.lifetime,0),1);b=Math.min(Math.floor(16*b),15);DrawRect(0,0,c.w,c.h,c.c+"0123456789ABCDEF".split("")[b]);PopMatrix();c.x+=c.vx;c.y+=c.vy;c.a+=c.va;c.w+=c.vw;c.h+=c.vh;c.t++;c.t>=c.lifetime&&(particles.splice(a,1),--a)}},PlayerStateIdle=0,PlayerStateBelly=1,PlayerStateHit=2,PlayerStateDead=3,PlayerStateIntro=
4,BellyBounceTime=45,BellyBounceAttackTime=8,PlayerHitTime=20,Player=function(){this.pos=new V2(200,-280);this.bellyOffset=new V2(0,0);this.bellyOffset.xLast=0;this.bellyOffset.yLast=0;this.dizzyVFXTimer=void 0;this.Reset()};Player.prototype.Reset=function(){this.health=3;this.score=0;this.isHighScore=!1;this.healthTimer=15;clearInterval(this.dizzyVFXTimer)};
...
Player.prototype.OnBounce=function(a){this.hitConfirm=!0;this.bellyOffset.xLast=0;this.bellyOffset.yLast=0;this.bellyOffset.x=15;this.bellyOffset.y=15;this.score++;this.healthTimer--;a=localStorage.getItem("bigchamp.highscore");if(null==a||a<this.score)localStorage.setItem("bigchamp.highscore",this.score),this.isHighScore=!0;zzfx.apply(null,[,,235,,,.12,,.15,-4.2,-6.7,,,,1.4,,.1,,.85,.04]);0==this.score%10&&zzfx.apply(null,[,,714,.38,.15,1,4,4.72,.7,.2,,,,.1,.9,.2,,.89])};
Player.prototype.OnHit=function(a){a.isHeart?3>this.health&&(this.health++,this.bellyOffset.x=5,this.bellyOffset.y=5,zzfx.apply(null,[,,716,,.02,.13,,1.71,,,636,.03,,.1,,.1,,.58,.01])):0<this.health&&(this.health--,this.timer=PlayerHitTime,this.state=PlayerStateHit,this.bellyOffset.x=5,this.bellyOffset.y=5,zzfx.apply(null,[,,272,,,.16,3,2.99,-1.7,1.6,,,,1.4,-.7,.2,,.68,.03]))};
